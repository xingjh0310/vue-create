...
// 通过++的第一次修改, 将副作用函数添加到jobQueue中, 然后执行flushJob 异步队列中去, 这时候副作用函数不会先执行, 会继续先执行后面的 ++ 
// 第二次 ++ 的时候, obj.foo 已经变为了3, 这时候一次副作用函数都没有执行,先执行 jobQueue.add(), 但是由于jobQueue 是个Set数据结构, 具有去重的功能, 最终jobQueue只会存储一个副作用函数, 即当前的副作用函数
